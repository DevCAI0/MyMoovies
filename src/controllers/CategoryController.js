...
const CategoryController = {
  ...
  async update(req, res) {
    try {
      ...
      const checkCategoryExists = await db.query("SELECT * FROM category WHERE name = $1 AND id <> $2", [name, id]);

      if (checkCategoryExists.rows.length > 0) {
        return res.status(400).json({ message: "Uma categoria com esse nome já existe." });
      }

      // Agora, atualize a categoria apenas se o nome não estiver em uso
      const updateQuery = "UPDATE category SET name = $1 WHERE id = $2 RETURNING *";
